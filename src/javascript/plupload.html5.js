...
(function(window, document, plupload, undef) {
	...
	plupload.runtimes.Html5 = plupload.addRuntime("html5", {
		...
		init : function(uploader, callback) {
			var self = this, features, xhr, dropElements;
			...
			uploader.bind("PostInit", function() {
				var dropElements = [], value = uploader.settings.drop_element, type;
				...
				if ( '[object String]' == type ) {
					dropElements.push( document.getElementById(value) );
				} else if ( type.indexOf('HTML') > -1 ) { //DOM element
					dropElements.push( value );
				} else if ( '[object Array]' == type ) {
					plupload.each( value, function(val) {
						if ( !val )
							return;

						if ( typeof(val) == 'string' )
							dropElements.push( document.getElementById(val) );
						else
							dropElements.push( val );
					});
				}
				...
				if ( dropElements.length ) {
					self.dropElements = dropElements;
						...
						plupload.each( dropElements, function(dropElm) {
			...
			uploader.bind('Destroy', function(up) {
				...
				if (self.dropElements) {
					plupload.each( dropElements, function(dropElm) {
						plupload.removeAllEvents(self.dropElm, up.id);
					});
				}
